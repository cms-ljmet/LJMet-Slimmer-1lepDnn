...
void step1::Loop() 
{
  ...
  // ----------------------------------------------------------------------------
   ...
   outputTree->Branch("theJetAK8MatchedTruth_PtOrdered",&theJetAK8MatchedTruth_PtOrdered);
  ...
  // ----------------------------------------------------------------------------
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	// Updated info from the TWiki CMS/TopSystematics#pt_top_Reweighting, NOTE goes to at least pT = 750
      ...
      TLorentzVector Wlv_1, Wlv_2, Wlv, lvTop, top_lv, lvXTF;
      if(DETtmp >= 0) {
	...
	isHadronicTprime = true;
      ...
      */
      ...
      // T' decay
      // ----------------------------------------------------------------------------                                                                                                           
      if(theJetAK8Pt_JetSubCalc_PtOrdered.size() > 2){
	float deltaR = 999;
	int bIndex = 999;
	for(unsigned int ijet=0; ijet < theJetPt_JetSubCalc_PtOrdered.size(); ijet++){
	  TLorentzVector jet_lv;
...
<<<<<<< HEAD
	...
	if(jets_lv.size() == 3){
	  bool allJetsCorrectlyMatched_BEST;
	  for(int ijet = 0 ; ijet < 3 ; ijet++){
	    if(dnn_largest_BestCalc_PtOrdered.at(jets_lv.at(ijet).second) == 2) { cout << "BEST found a Higgs" << endl; }
	  }
